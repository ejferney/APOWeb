        ...
        <aside className="hidden md:flex w-64 flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 h-full overflow-y-auto flex-shrink-0">
            <div className="p-6 flex flex-col h-full justify-between">
                <div className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <Link to="/service-logs" className={`px-3 py-2 rounded-lg flex items-center gap-3 cursor-pointer transition-colors ${isActive('/service-logs') ? 'bg-primary/10 text-primary' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
                            ...
                            <p className="text-sm font-medium">My Credits</p>
                        </Link>
                        ...
                        <Link to="/family-tree" className={`px-3 py-2 rounded-lg flex items-center gap-3 cursor-pointer transition-colors ${isActive('/family-tree') ? 'bg-primary/10 text-primary' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
                            <span className="material-symbols-outlined text-xl">account_tree</span>
                            <p className="text-sm font-medium">Family Tree</p>
                        </Link>
                        ...
                        {isOfficer && (
                            <>
                                ...
                                <Link to="/reports" className={`px-3 py-2 rounded-lg flex items-center gap-3 cursor-pointer transition-colors ${isActive('/reports') ? 'bg-primary/10 text-primary' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
                                    <span className="material-symbols-outlined text-xl">monitoring</span>
                                    <p className="text-sm font-medium">Requirements Report</p>
                                </Link>
                                {(user?.position === 'Pledge Educator' || user?.position === 'President' || user?.role === 'admin') && (
                                    <Link to="/family-manager" className={`px-3 py-2 rounded-lg flex items-center gap-3 cursor-pointer transition-colors ${isActive('/family-manager') ? 'bg-primary/10 text-primary' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
                                        <span className="material-symbols-outlined text-xl">diversity_2</span>
                                        <p className="text-sm font-medium">Manage Families</p>
                                    </Link>
                                )}
                            </>
                        )}
                ...
                <div className="flex flex-col gap-4 border-t border-slate-100 dark:border-slate-800 pt-4 mt-auto">
                    <Link to="/profile" className={`flex items-center gap-3 px-2 py-1.5 rounded-lg transition-colors ${isActive('/profile') ? 'bg-primary/10' : 'hover:bg-slate-100 dark:hover:bg-slate-800'}`} title="Edit profile">
                        ...
                        <span className="material-symbols-outlined text-slate-400 text-[18px] ml-auto">edit</span>
                    </Link>
